...
**A private messenger for people who'd rather be off their phone.**

`v1.0.0` · Android · MPL-2.0 · made by [ArtFacility](https://github.com/ArtFacility)

</div>

> ⚠️ **Heads up:** this is the first semi-stable release. It works and I daily-drive it, but it's still very much a work in progress — expect rough edges, missing bits, and the occasional weird bug. Issues and suggestions welcome.

---

WiltKey is an encrypted chat app built on a slightly old-fashioned idea: that the people worth talking to are usually the ones you can actually meet. So instead of accounts, phone numbers, or "add me by username," you swap keys **in person over Bluetooth**. No sign-up, no profile in the cloud, nothing tying your identity to a server somewhere.

The encryption is a one-time pad — the kind that's mathematically unbreakable when used right — and the relay server in the middle is deliberately blind. It shoves encrypted blobs from A to B and keeps **zero** chat history. If someone's offline, their message waits in a queue for 24 hours and then it's gone, delivered or not.

And the whole thing is designed to *not* hold you hostage. No infinite feed, no streaks, no "someone is typing…" dopamine loops. It's a tool to coordinate seeing real people, and the sooner you put it down, the better it's doing its job.
...
- **It's yours to make weird.** Multiple themes (a cozy Garden one, a neon Cyberpunk one, a clean Paper-ink one), pixel-art avatars you draw yourself, and per-chat custom emojis.
...
- **Custom emojis per chat** — make a `:donkeyd:` or a `:mogged:`, use it inline with `:name:`.
...
- **Localized** into several languages (and this is the easiest place to help out — see below).
...
It's at **1.0.0** — the first version I'm comfortable letting other people poke at. Core messaging, groups, theming, and the security basics are in and working. Plenty is still planned:
...
- 📍 **Meetup discovery** — create small local events near you so people with shared interests can actually meet up in person (this one's the heart of where WiltKey is headed).
- …and a steady stream of smaller stuff: more themes, polish, and the special-action animations that are still stubbed out.

No promises on timing — it's a solo project I work on around life.
...
If you speak a language WiltKey already supports and something reads awkwardly — fix it. If you speak one it *doesn't* support yet — adding it is very welcome. Open a PR or just an issue with suggestions; I'd rather have a slightly rough translation from a native speaker than a perfect one from a machine.

Beyond that, bug reports and feature thoughts are great. Just keep in mind the project has a few non-negotiables (no plaintext at rest, the server stays blind, one-time pads never get reused, themes stay token-driven, and nothing should be designed to keep you glued to the screen). The [docs](https://wiltkey.org/docs/) spell those out if you're curious.
